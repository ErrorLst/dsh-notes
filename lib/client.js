	...
	factory: (require) => {
		...
		if (typeof PerformanceObserver !== "undefined") try {
			new PerformanceObserver((list) => {
				for (const entry of list.getEntries()) if (entry.duration >= 50) console.debug("[dsh-notes][longtask]", Math.round(entry.duration), "ms @", Math.round(entry.startTime));
			...
			useEffect(() => {
				...
				const load = () => {
					...
					fetch(`/api/dsh-notes${query}`, { cache: "no-store" }).then((response) => response.json()).then((result) => {
						if (!alive) return;
						console.debug("[dsh-notes] fetch resolved in", Math.round(performance.now() - t0), "ms; ok=", result.ok, result.ok ? "ws todos=" + (result.workspace ? result.workspace.todos?.length ?? 0 : "null") : "err=" + result.error);
						if (result.ok === true) {
							const next = {
