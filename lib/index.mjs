	...
	* 必须经全局瀑布监听显式应用（与 scard-select-model 同一路径）。 */
	...
	async function scardClear() {
		...
		if (agent !== void 0 && agent.status === "running") {
			try {
				if (typeof agent.cancel === "function") agent.cancel(/* @__PURE__ */ new Error("常驻会话已清空"));
			...
			while (agent.status === "running" && Date.now() < deadline) await new Promise((resolve) => setTimeout(resolve, 100));
		}
		try {
			if (workspaceRegistry !== void 0) await workspaceRegistry.archiveSession(id);
		} catch (error) {
