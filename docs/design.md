...
>
> 变更记录：
> 0.4.24 —— **修复悬浮提示卡住**：悬停折叠胶囊显示「展开小记」提示后点击展开——触发提示的按钮随重渲染被卸载，mouseout 事件随之消失，提示框永远停在旧位置（折叠时算出的坐标 ≈ 展开后常驻会话卡片的左侧），表现为「展开后常驻会话左边一直显示展开小计」。修复：`collapse`/`expand` 先 `hideTip()`，并给竖栏根加 `onPointerDown` 兜底隐藏（覆盖一切点击后卸载按钮的路径）。
...
> 0.4.22 —— 待办详情卡片**自动保存**：去掉底部「保存/取消」按钮——输入防抖 600ms 自动落盘（与随记一致），关闭（× / 点遮罩 / ESC / 切换工作区）时冲刷未保存内容；标题清空则不保存（保留原标题）；头部显示「保存中…/已保存」状态，元信息行的更新时间随保存刷新。原型同步。
